...
import com.auth.server.model.Authorization;
import com.auth.server.repository.AuthorizationRepository;
...
public class OAuth2AuthorizationServiceImpl implements OAuth2AuthorizationService {
	...
	private OAuth2Authorization toOAuth2Authorization(Authorization authorization) {
		...
		if(authorization.getAuthorizationCodeValue() != null) {
			System.out.println("Authorization code value => " + authorization.getAuthorizationCodeValue());
			System.out.println(parseMap(authorization.getAuthorizationCodeMetadata()));
			...
			System.out.println("Not a authorization code");
		}
		...
		if(authorization.getAccessTokenValue() != null) {
			...
			System.out.println("Not a Access token");
		}
		...
		if (authorization.getOidcIdTokenValue() != null) {
			...
			System.out.println("Not a OIDC Token");
		}
		/**
		 ...
		 * why this arises.
		 */
		/** Fix for acess token getting flow */
		String userPass = null;
		try {
			AppUser appUser = AppUser.toAppUser(appUserService
										.findByUserId(authorization.getPrincipalName()));
			userPass = appUser.getPassword();
		} catch (UserNotFoundException e) {
			System.out.println("User Not found with the principal name," +
					" So settings password as null");
		}
		...
		builder.attribute(Principal.class.getName(), test);
